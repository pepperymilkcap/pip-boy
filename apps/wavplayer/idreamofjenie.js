...
const COLOR_GREEN = 2; // Using color 2 for brighter green
const COLOR_BLACK = 0;
...
function drawMenu() {
  ...
  G.setColor(COLOR_GREEN).fillRect(LEFT_MARGIN, UNDERLINE_Y, G.getWidth() - LEFT_MARGIN, UNDERLINE_Y + 3);
  ...
  filesOnPage.forEach(function(name, i) {
    ...
    if (actualIndex === selected) {
      var boxPadding = (LINE_HEIGHT - 23) / 2;
...
function frameLoop() {
  ...
  // IMPORTANT: Only redraw the screen if audio is not playing
...
function handlePlayButton() {
  ...
  drawMenu(); // Draw the "PLAYING" message once
  
  statusTimeout = setTimeout(function() {
    playMsgLines = [];
    statusTimeout = null;
    needsRedraw = true; // Request a redraw, which will happen after playback stops
  }, 4000);
}
