...
//                                                                 Low Level Comms
  ...
  */
  ...
  /** Write data and wait for a specific termination character (e.g., '>' prompt)
    instead of relying on timeouts. This is more reliable for commands like g.dump()
    that send large amounts of data with unpredictable timing.
    
    options = {
      terminator : string // character(s) to wait for to indicate completion (default: '>')
      maxWaitTime : number // maximum time to wait in ms (default: 300000 = 5 minutes)
    }
  */
  writeUntilTerminator : (data, options) => {
    if (data===undefined) throw new Error("Comms.writeUntilTerminator(undefined) called!")
    options = options||{};
    const terminator = options.terminator || '>';
    const maxWaitTime = options.maxWaitTime || 300000; // 5 minutes default
    
    return new Promise((resolve, reject) => {
      const connection = Comms.getConnection();
      if (!connection) {
      ...
      timeoutHandle = setTimeout(() => {
        ...
        reject(new Error(`Timeout waiting for terminator '${terminator}' after ${maxWaitTime}ms. Received: ${receivedData.length} characters`));
      }, maxWaitTime);
      
      // Set up data handler to accumulate data until terminator is found
      dataHandler = (chunk) => {
        receivedData += chunk;
        
        // Check if we've received the terminator
        if (receivedData.includes(terminator)) {
          // Clean up
          clearTimeout(timeoutHandle);
          connection.removeListener('data', dataHandler);
          
          // Extract the data before the terminator
          const terminatorIndex = receivedData.lastIndexOf(terminator);
          const resultData = receivedData.substring(0, terminatorIndex);
          
          console.log(`<COMMS> Received terminator '${terminator}' after ${resultData.length} characters`);
          resolve(resultData);
        }
      };
      ...
      // Send the command
      if (typeof UART !== "undefined") {
        // For UART, write directly
        connection.write(data);
      } else {
        // For Puck, we need to use the Puck write mechanism but bypass the timeout handling
